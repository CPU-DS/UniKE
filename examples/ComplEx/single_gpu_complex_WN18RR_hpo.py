"""
`ComplEx-WN18RR-single-gpu-wandb <single_gpu_complex_WN18RR_wandb.html>`_ ||
**ComplEx-WN18RR-single-gpu-hpo**
...
这一部分介绍如何用一个 GPU 在 ``WN18RR`` 知识图谱上寻找 ``ComplEx`` :cite:`ComplEx` 的超参数。
...
data_loader_config.update({
    'in_path': {
        'value': os.path.join(os.path.dirname(__file__), '../../benchmarks/WN18RR/')
    },
    'threads': {
        'value': 2
    },
...
# 定义模型超参数优化范围
# ---------------------------------------------------------
# :py:func:`unike.module.model.get_complex_hpo_config` 返回了
# :py:class:`unike.module.model.ComplEx` 的默认超参数优化范围。
...
# 定义损失函数超参数优化范围
# ---------------------------------------------------------
# :py:func:`unike.module.loss.get_softplus_loss_hpo_config` 返回了
# :py:class:`unike.module.loss.SoftplusLoss` 的默认超参数优化范围。
...
######################################################################
# .. Note:: 上述代码的运行日志可以从 `此处 </zh-cn/latest/_static/logs/examples/ComplEx/single_gpu_complex_WN18RR_hpo.txt>`_ 下载。
# .. Note:: 上述代码的运行报告可以从 `此处 </zh-cn/latest/_static/pdfs/examples/ComplEx/ComplEx超参数搜索示例（一）.pdf>`_ 下载。
