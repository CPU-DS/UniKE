"""
`TransD-FB15K237-single-gpu-wandb <single_gpu_transd_FB15K237_wandb.html>`_ ||
**TransD-FB15K237-single-gpu-hpo**
...
这一部分介绍如何用一个 GPU 在 ``FB15K237`` 知识图谱上寻找 ``TransD`` :cite:`TransD` 的超参数。
...
# 定义模型超参数优化范围
# ---------------------------------------------------------
# :py:func:`unike.module.model.get_transd_hpo_config` 返回了
# :py:class:`unike.module.model.TransD` 的默认超参数优化范围。
...
######################################################################
# .. Note:: 上述代码的运行日志可以从 `此处 </zh-cn/latest/_static/logs/examples/TransD/single_gpu_transd_FB15K237_hpo.txt>`_ 下载。
