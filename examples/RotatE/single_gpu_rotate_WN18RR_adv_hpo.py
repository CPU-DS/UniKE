...
**RotatE-WN18RR-single-gpu-adv-hpo**
...
这一部分介绍如何用一个 GPU 在 ``WN18RR`` 知识图谱上寻找 ``RotatE`` :cite:`RotatE` 的超参数。
...
# 定义模型超参数优化范围
# ---------------------------------------------------------
# :py:func:`unike.module.model.get_rotate_hpo_config` 返回了
# :py:class:`unike.module.model.RotatE` 的默认超参数优化范围。
...
# 定义损失函数超参数优化范围
# ---------------------------------------------------------
# :py:func:`unike.module.loss.get_sigmoid_loss_hpo_config` 返回了
# :py:class:`unike.module.loss.SigmoidLoss` 的默认超参数优化范围。
...
######################################################################
# .. Note:: 上述代码的运行日志可以从 `此处 </zh-cn/latest/_static/logs/examples/RotatE/single_gpu_rotate_WN18RR_adv_hpo.txt>`_ 下载。
